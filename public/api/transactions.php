    ...
    if ($totalInstallments > 1) {
        ...
        for ($i = 1; $i <= $totalInstallments; $i++) {
            ...
            $stmt->execute([
                ...
                $data['categoryId'],
                $data['organizationId'],
                (($data['status'] ?? 'paid') === 'paid' && empty($data['due_date'])) ? $installmentDate : ($data['due_date'] ?? null),
                (($data['status'] ?? 'paid') === 'paid' && empty($data['payment_date'])) ? $installmentDate : ($data['payment_date'] ?? null),
                $data['status'] ?? 'paid',
                $data['is_fixed'] ?? 0,
                $parentTransactionId,
                $i,
        ...
        $stmt->execute([
            ...
            $data['categoryId'],
            $data['organizationId'],
            (($data['status'] ?? 'paid') === 'paid' && empty($data['due_date'])) ? $data['date'] : ($data['due_date'] ?? null),
            (($data['status'] ?? 'paid') === 'paid' && empty($data['payment_date'])) ? $data['date'] : ($data['payment_date'] ?? null),
            $data['status'] ?? 'paid',
            $data['is_fixed'] ?? 0,
            $data['reminderDays'] ?? null
        ]);
